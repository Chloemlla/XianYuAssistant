...
Mapper 基于 `AbstractMongoMapper` 与 `MongoTemplate`，实体通过 `@Document` 映射到 MongoDB 集合。

`MongoIdGenerator` 为仍使用 Long 业务 ID 的文档提供序列；复杂分页、条件更新和 claim 操作由专用 Mapper 显式实现。

`spring.data.mongodb.auto-index-creation=true` 配合实体上的 `@Indexed`/`@CompoundIndex` 建立关键索引。生产部署仍应在升级前备份，并核对唯一索引创建是否因历史重复数据失败。
