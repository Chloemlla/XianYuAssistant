...
Spring MVC 控制器
  ...
  -> Spring AI / MongoDB Atlas Vector Search
...
- Spring Data MongoDB 由 Spring Boot 3.5.7 管理，业务 Mapper 基于 MongoTemplate 封装。
- MongoDB 8.0+ 存储业务集合；AI 知识库使用 MongoDB Atlas Vector Search。
...
- Spring AI 1.1.4、OpenAI 兼容模型客户端和 MongoDB Atlas Vector Store 提供 RAG 能力。
...
业务数据与向量文档位于 `SPRING_DATA_MONGODB_URI` 指向的 MongoDB；应用容器只持有日志及运行期临时文件。MongoDB 必须独立持久化并执行数据库级备份。
...
当前发布版本为 2.0.3。Maven、前端 manifest、锁文件、应用版本和 Docker 构建参数由 GitHub Actions 一致性门禁校验。
